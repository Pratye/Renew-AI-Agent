...
class VectorStore:
    """Vector database using FAISS for storing and retrieving renewable energy data for chat"""
    ...
    def __init__(self):
        ...
        self.index_paths = {
            ...
            "chat": os.path.join(os.getcwd(), "data", "vector_store", "chat.index")
        }
        ...
        for data_type, path in self.index_paths.items():
            if os.path.exists(path):
                try:
                    ...
                    self.logger.error(f"Error loading index for {data_type}: {e}")
                    # Create a new L2 index
                    self.indices[data_type] = faiss.IndexFlatL2(384)  # 384 is the dimension of all-MiniLM-L6-v2 embeddings
                    self.logger.info(f"Created new FAISS index for {data_type} after load failure")
            else:
                # Create a new L2 index
                self.indices[data_type] = faiss.IndexFlatL2(384)  # 384 is the dimension of all-MiniLM-L6-v2 embeddings
    ...
    def store_chat_data(self, question: str, answer: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        """Store chat Q&A pairs for future retrieval."""
        try:
            data = {
                "question": question,
                "answer": answer,
            ...
            return self.store_data(data, "chat")
        except Exception as e:
            self.logger.error(f"Error storing chat data: {e}")
            return False
    ...
    def query_data(self, query: str, data_type: str, n_results: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if data_type not in self.indices or self.indices[data_type].ntotal == 0:
                self.logger.warning(f"No data in index for {data_type}")
                return []
            
            # Search FAISS index
            D, I = self.indices[data_type].search(
                query_embedding.reshape(1, -1),
                min(n_results, self.indices[data_type].ntotal)
            )
    ...
    def query_chat(self, query: str, n_results: int = 3) -> List[Dict[str, Any]]:
        """Query the vector store for similar chat Q&A pairs."""
        try:
            return self.query_data(query, "chat", n_results)
        except Exception as e:
            self.logger.error(f"Error querying chat data: {e}")
            return []
